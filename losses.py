...
class DiceLoss2(nn.Module):
    ...
    def forward(self, input, target):
        ...
        dice_coefficient = (2.0 * intersection + self.smooth) / (cardinality + self.smooth)
...
class DiceLoss(nn.Module):
    def __init__(self, num_classes, ignore_index=-1, smooth=1e-6):
    ...
    def forward(self, pred, target):
        ...
        # transpose inputs to shape (batch_size, num_points, num_classes)
        ...
        if self.ignore_index != -1:
            ...
            ignore_mask = target[..., self.ignore_index] == 1
            target[ignore_mask] = 0
            
            
            # pred[ignore_mask, ...] = torch.zeros(self.num_classes)
            
        # calculate intersection and union
        intersection = torch.sum(pred * target, dim=2)
        cardinality = torch.sum(pred + target, dim=2)
