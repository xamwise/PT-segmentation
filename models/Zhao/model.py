    ...
    def __init__(self, in_planes, out_planes=None, num_shape_class=None):
        ...
        if out_planes is None:
            ...
            if num_shape_class is not None:
                self.linear1 = nn.Sequential(nn.Linear(2 * in_planes + 1024, in_planes),
    ...
    def forward(self, pxo1, pxo2=None, y=None):
        if pxo2 is None:
            ...
            for i in range(o.shape[0]):
                ...
                x_b = torch.cat((x_b, self.linear2(x_b.sum(0, True) / cnt).repeat(cnt, 1),
                                 self.linear3(y_onehot).repeat(cnt, 1)), dim=1)
                x_tmp.append(x_b)
            x = torch.cat(x_tmp, 0)
            x = self.linear1(x)
        else:
...
class PointTransformerSegBase(nn.Module):
    def __init__(self, block, blocks, cfg, in_channels=6):#, in_channels=6, num_classes=50, num_shape_classes=None):#, knn_sampling=16, num_points=1024):
        ...
        self.knn_sampling = cfg.model.knn_sampling
        self.num_points = cfg.num_point
        stride, nsample = [1, 4, 4, 4, 4], [8, self.knn_sampling, self.knn_sampling, self.knn_sampling, self.knn_sampling]
        ...
        self.dec5 = self._make_dec(block, planes[4], 1, share_planes, num_shape_classes=self.num_shape_classes,
                                   nsample=nsample[4], is_head=True)  # transform p5
        ...
        self.cls = nn.Sequential(nn.Linear(planes[0], planes[0]),
                                 ...
                                 nn.ReLU(inplace=True), nn.Linear(planes[0], self.num_classes))
...
class PointTransformerSeg(PointTransformerSegBase):
    def __init__(self, cfg):
        super(PointTransformerSeg, self).__init__(Bottleneck, [1, 1, 1, 1, 1], cfg)
